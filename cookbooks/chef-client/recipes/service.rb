...
when "init"
  ...
  dist_dir = value_for_platform(
    ...
    ["amazon", "redhat", "centos", "fedora","scientific"] => { "default" => "redhat"}
  )

  conf_dir = value_for_platform(
    ["ubuntu", "debian"] => { "default" => "default" },
    ["amazon", "redhat", "centos", "fedora","scientific"] => { "default" => "sysconfig"}
  )

  chef_version = node.chef_packages.chef["version"]

  if node[:chef_packages][:chef][:chef_root] =~ /^\/opt\/opscode\/embedded/ then
    init_content = IO.read("#{node[:chef_packages][:chef][:chef_root]}/../distro/#{dist_dir}/etc/init.d/chef-client")
    conf_content = IO.read("#{node[:chef_packages][:chef][:chef_root]}/../distro/#{dist_dir}/etc/#{conf_dir}/chef-client")
  else
